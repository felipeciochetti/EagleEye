package br.com.eagleeye.complete;

import java.util.ArrayList;
import java.util.List;

import br.com.eagleeye.jpa.Ncm;
import br.com.eagleeye.repositorio.Ncms;

public class NcmComplete {

	private Ncms repositorio;
	
	public NcmComplete(Ncms repositorio){
		this.repositorio = repositorio;
	}
	
	
	public List<Ncm> execute(String query){
		if (query.length() > 0) {
			if ("0123456789".indexOf(query.substring(0, 1)) >= 0) {
				return repositorio.listarCodigo(query, 30);
			} else {
				return repositorio.listarNome(query, 30);
			}
		}
		return new ArrayList<Ncm>();  
	}

	
}
